...
if ($_SESSION['logged_in'] == true && ($_SESSION['recht'] == "A") || ($_SESSION["recht"] == "M") && $_SESSION['department'] == "Finance") {
    ...
    $query = "SELECT ors.id, o.datum, p.afbeelding, p.naam, ors.aantal, o.totaalprijs, p.leverancier, ors.status FROM orderregels ors JOIN orders o on ors.order_id = o.id JOIN products p on p.id = ors.product WHERE ors.status = 'toegewezen'";
...
?>
                 <tr>
                    ...
                    <?php if($status != "betaald" && $status != "afgewezen" && $status != "authorisatie nodig"){?>
                    <td><a target="_self"
